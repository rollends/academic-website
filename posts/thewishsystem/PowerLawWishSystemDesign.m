...
sol = simulannealbnd(@(a) Cost2Function(a(1), a(2)), [0.2324 0.5/100], [1e-4 1e-4], [1 - 1e-4, 1 - 1e-4], options)

%% Final Distribution
dist = FinalDistributionPowerLaw(sol(1), sol(2));
...
function cost = Cost2Function(arvalue, auvalue)
    dist = FinalDistributionPowerLaw(arvalue, auvalue);
    cost = 0;
    ...
    % Reaching State 45 should be half that of State 1.
    p1 = 0;
    p2 = 0;
    for xr = 0:9
        j = StateToIndex(xr, 1);
        ...
        j = StateToIndex(xr, 45);
        p2 = p2 + dist(j);
    end
    cost = cost + abs((2*p2 - p1)/p1);
